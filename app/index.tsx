...
=======
>>>>>>> b5f310b067755afdf13a79cdcdc9d6ede2dc10f3
import WeatherIcon from '../components/WeatherIcon';
import * as SecureStore from '../utils/storage';
import { fetchUserLocation, fetchWeatherForLocation, mapIconCodeToIonicon, WeatherData } from '../utils/weather';
            ...
            <SafeAreaView style={{ flex: 1 }}>
                <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
                    ...
                    <View style={styles.card}>
                        ...
                        <View style={styles.divider} />
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.hourlyList}>
                            {weatherData?.hourlyForecast && weatherData.hourlyForecast.length > 0 ? weatherData.hourlyForecast.map((hour: any, idx: number) => {
                                const date = new Date(hour.timestamp);
                                const timeString = idx === 0 ? 'Now' : date.toLocaleTimeString([], { hour: 'numeric', hour12: true }).replace(' ', '');
                                const iconName = mapIconCodeToIonicon(hour.iconCode);
                                return (
                                    <View key={`hourly-${idx}`} style={styles.hourlyItem}>
                                        <Text style={styles.hourlyTime}>{timeString}</Text>
                                        <WeatherIcon name={iconName} size={28} />
